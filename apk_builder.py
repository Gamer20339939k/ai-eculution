...
def main() -> None:
    ...
    if engine == "beeware":
        try:
            ...
            src_text = ""
        if "import tkinter" in src_text:
            fallback = root / "android-chaquopy" / "app" / "src" / "main" / "python" / "ai_leanr_walk.py"
            if fallback.exists():
                print("Hinweis: tkinter-Quelle für BeeWare erkannt. Nutze Android-freundliche Quelle:", fallback)
                src = fallback
            else:
                print("Warnung: tkinter-Import erkannt. BeeWare-App kann abstürzen.")
